...
import { Wrench, Mail, Lock, ArrowRight, Loader2, RefreshCw, User, CheckCircle, X } from 'lucide-react';
...
const Login = () => {
  ...
  const [selectedGoogleAcc, setSelectedGoogleAcc] = useState({
    name: 'G. Sasimanth Reddy',
    email: 'gsasimanthreddy@gmail.com',
    role: 'user',
    avatar: '👨'
  });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-5 relative">
            <button
              ...
              className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100 transition cursor-pointer border-none bg-transparent"
            >
              <X size={18} />
            </button>
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
              ...
              <div>
                <h3 className="font-extrabold text-slate-900 text-sm">Choose Google Account</h3>
                <p className="text-[11px] text-slate-500 font-semibold">To sign in and continue to Fixvo</p>
              </div>
            </div>

            <div className="space-y-2">
              {[
                { name: 'G. Sasimanth Reddy', email: 'gsasimanthreddy@gmail.com', role: 'user', avatar: '👨' },
                { name: 'Amit Verma (Tech Pro)', email: 'amit.verma.tech@gmail.com', role: 'technician', avatar: '👨‍🔧' },
                { name: 'Fixvo Admin Console', email: 'admin.executive@fixvo.in', role: 'admin', avatar: '⚡' }
              ].map(acc => {
                const isSelected = selectedGoogleAcc.email === acc.email;
                return (
                  <div
                    key={acc.email}
                    onClick={() => setSelectedGoogleAcc(acc)}
                    className={`p-3.5 rounded-2xl border-2 transition-all cursor-pointer flex items-center justify-between ${
                      isSelected ? 'border-blue-600 bg-blue-50/60 shadow-xs' : 'border-slate-200 hover:border-slate-300 bg-white'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-extrabold text-slate-900 text-xs">{acc.name}</h4>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  navigate(selectedGoogleAcc.role === 'admin' ? '/admin-dashboard' : selectedGoogleAcc.role === 'technician' ? '/technician-dashboard' : '/dashboard');
                } catch (err) {
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-extrabold py-3 rounded-2xl text-xs uppercase tracking-wider transition-all cursor-pointer border-none shadow-md"
            >
              Continue as {selectedGoogleAcc.name.split(' ')[0]} →
            </button>
          </div>
        </div>
      )}
