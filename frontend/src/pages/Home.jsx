        ...
        <nav className="relative pointer-events-auto bg-[#0B0F19]/70 backdrop-blur-2xl border border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.3)] rounded-[2rem] w-full max-w-5xl transition-all duration-500">
          <div className="px-4 sm:px-6">
            <div className="flex justify-between items-center h-16 sm:h-20">
              ...
              <div className="hidden lg:flex flex-1 max-w-sm mx-4 relative" ref={topSearchRef}>
                ...
                {/* Top Nav Desktop Suggestions Dropdown */}
                <AnimatePresence>
                  {isSearchFocused && (
                    <motion.div 
                      ...
                      className="absolute left-0 right-0 top-full mt-2 bg-[#101524]/98 border border-white/10 backdrop-blur-xl rounded-2xl shadow-2xl p-3 z-50 text-left overflow-hidden max-h-[360px] overflow-y-auto"
                    >
                      {!searchQuery.trim() && (
                        <div className="space-y-3">
                          {searchHistory.length > 0 && (
                            <div>
                              <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1.5">
                                ...
                                <button onClick={clearHistory} className="hover:text-white cursor-pointer bg-transparent border-0">Clear</button>
                              </div>
                              <div className="space-y-0.5">
                                {searchHistory.map((item, i) => (
                                  <div 
                                    ...
                                    className="flex justify-between items-center px-2.5 py-1.5 rounded-xl text-slate-300 hover:text-white hover:bg-white/5 transition cursor-pointer text-xs font-semibold"
                                  >
                                    <span>{item}</span>
                                    <button 
                          ...
                          <div>
                            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500 flex items-center gap-1 mb-1.5">
                              <Sparkles size={11} className="text-cyan-400" /> Popular Searches
                            </p>
                            <div className="flex flex-wrap gap-1.5">
                              {["AC Repair", "Washing Machine", "Electrician", "RO Installation"].map((pop, i) => (
                                <button
                                  key={i}
                                  onClick={() => handleSearchSelect(pop, 'service', services.find(s => s.name.toLowerCase() === pop.toLowerCase())?.id || '')}
                                  className="px-2.5 py-1 bg-white/5 border border-white/5 hover:border-blue-500/20 text-slate-300 hover:text-white hover:bg-blue-600/10 text-[10px] font-bold rounded-full transition cursor-pointer"
                                >
                                  {pop}
                                </button>
                              ))}
                        ...
                        <div className="space-y-3">
                          {hasSuggestions ? (
                            <>
                              {suggestions.services.length > 0 && (
                                <div>
                                  <p className="text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1">Services</p>
                                  <div className="space-y-0.5">
                                    {suggestions.services.map(s => (
                                      <button
                                        key={s.id}
                                        onClick={() => handleSearchSelect(s.name, 'service', s.id)}
                                        className="w-full text-left px-2.5 py-1.5 rounded-xl text-slate-300 hover:text-white hover:bg-white/5 flex items-center justify-between text-xs font-bold transition cursor-pointer"
                                      >
                                        <span className="truncate">{s.name}</span>
                                        <span className="text-[9px] font-black text-cyan-400 px-1.5 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/20 uppercase tracking-wide">{s.price}</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                              ...
                              {suggestions.categories.length > 0 && (
                                <div>
                                  <p className="text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1">Categories</p>
                                  <div className="space-y-0.5">
                                    {suggestions.categories.map(c => (
                                      <button
                                        key={c.id}
                                        onClick={() => handleSearchSelect(c.name, 'category', c.id)}
                                        className="w-full text-left px-2.5 py-1.5 rounded-xl text-slate-300 hover:text-white hover:bg-white/5 flex items-center gap-2 text-xs font-bold transition cursor-pointer"
                                      >
                            ...
                            <div className="py-4 text-center text-xs text-slate-500 font-semibold italic">
