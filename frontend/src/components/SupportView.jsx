    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center pb-4 border-b border-slate-200">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium">Browse FAQs or raise a support query ticket</p>
        </div>
        <button
      ...
      {showTicketForm && (
        <form onSubmit={handleRaiseTicket} className="bg-white p-6 rounded-2xl border border-slate-200 space-y-4 animate-in slide-in-from-top-2 duration-200">
          <h3 className="font-extrabold text-sm text-slate-800">Submit Support Ticket</h3>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block ml-1">Category</label>
              <select
                value={ticketForm.category}
                onChange={(e) => setTicketForm({ ...ticketForm, category: e.target.value })}
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-semibold text-white text-xs focus:border-indigo-500"
              >
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block ml-1">Subject</label>
              <input
                required
                type="text"
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block ml-1">Message Description</label>
            <textarea
              required
              rows={3}
              ...
              className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none font-semibold text-white text-xs focus:border-indigo-500 resize-none"
            />
          </div>
          <button
            type="submit"
      ...
      {supportTickets.length > 0 && (
        <div className="space-y-3 pt-2">
          ...
          <div className="space-y-2 bg-slate-100 border border-slate-200 rounded-2xl p-4 divide-y divide-white/5">
            {supportTickets.map((tkt) => (
              <div key={tkt.id} className="py-3 first:pt-0 last:pb-0">
                ...
                <h4 className="font-bold text-slate-900 text-xs sm:text-sm mt-1">{tkt.subject}</h4>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
        <a
          ...
          className="bg-white p-5 rounded-2xl border border-slate-200 hover:border-indigo-500/30 transition-all flex flex-col items-center text-center justify-between min-h-[140px] text-slate-700 no-underline cursor-pointer group"
        >
          <MessageCircle size={28} className="text-[#25D366] group-hover:scale-110 transition-transform" />
          <div>
            <h4 className="font-extrabold text-slate-850 text-xs tracking-tight">WhatsApp Chat</h4>
        ...
        >
          ...
          <div>
            <h4 className="font-extrabold text-slate-850 text-xs tracking-tight">Call Support</h4>
        ...
        >
          ...
          <div>
            <h4 className="font-extrabold text-slate-850 text-xs tracking-tight">Email Help</h4>
      ...
      <div className="space-y-3 pt-4">
        <h3 className="font-extrabold text-sm text-slate-700 tracking-tight uppercase tracking-wider">Frequently Asked Questions</h3>
          ...
          {faqs.map((faq, idx) => (
            <div key={idx} className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              <button
                ...
                className="w-full px-5 py-4 flex justify-between items-center text-left text-xs sm:text-sm font-extrabold text-slate-800 outline-none hover:bg-slate-100 cursor-pointer border-none bg-transparent"
              >
                <span>{faq.q}</span>
                {openFaq === idx ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
              </button>
              {openFaq === idx && (
                <div className="px-5 pb-5 pt-1 text-xs text-slate-500 font-medium leading-relaxed border-t border-slate-200/10 animate-in fade-in duration-200">
                  {faq.a}
                </div>
              )}
            </div>
