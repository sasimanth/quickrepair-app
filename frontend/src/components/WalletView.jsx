  ...
  return (
    <div className="space-y-6">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-xs text-slate-400 mt-1 font-medium">Manage and check your digital payments & cashback</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-indigo-950 via-[#1E293B] to-slate-950 p-6 rounded-3xl border border-indigo-500/20 relative overflow-hidden flex flex-col justify-between min-h-[160px] shadow-lg shadow-indigo-900/10">
          ...
          <div className="flex gap-4 text-[10px] text-slate-400 font-bold border-t border-white/5 pt-3.5 mt-4">
        ...
        {/* Add Money Form */}
        <form onSubmit={handleAddWalletMoney} className="bg-slate-900/40 p-5 rounded-3xl border border-white/5 flex flex-col justify-between space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block ml-1">Add Money</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-4 py-3 bg-slate-950 border border-white/10 rounded-xl outline-none font-bold text-white text-sm focus:border-indigo-500 transition-all"
              />
            </div>
      ...
      <div className="space-y-4 pt-4">
        <h3 className="font-extrabold text-sm text-slate-300 tracking-tight uppercase tracking-wider">Transaction History</h3>
        <div className="bg-slate-900/40 rounded-3xl border border-white/5 overflow-hidden divide-y divide-white/5">
          {transactions.length === 0 ? (
            <div className="py-8 text-center text-slate-500 font-bold text-xs">No transactions recorded.</div>
          ) : (
            transactions.map((tx) => (
              <div key={tx.id} className="p-4 flex justify-between items-center hover:bg-slate-900/10 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-white text-xs sm:text-sm">{tx.desc}</h4>
                    <span className="text-[9px] text-slate-500 font-semibold block mt-0.5">
                      {new Date(tx.date).toLocaleDateString()} • {new Date(tx.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                </div>
                <span className={`font-black text-sm ${tx.type === 'credit' ? 'text-emerald-450' : 'text-slate-400'}`}>
