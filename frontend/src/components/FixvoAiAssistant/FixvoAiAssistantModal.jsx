...
import { 
  ...
  ArrowRight, Edit3, Loader2, AlertCircle, RefreshCw, ChevronRight, UserCheck
} from 'lucide-react';
...
const FixvoAiAssistantModal = ({ 
  ...
  const [messages, setMessages] = useState([
    {
      id: 'init_1',
      sender: 'ai',
      text: `Hello ${userProfile?.name?.split(' ')[0] || 'there'}! 👋 I am your Fixvo AI Service Assistant.\n\nTell me what service you need or describe your problem naturally (e.g. "My AC is not cooling, need a technician tomorrow morning in Madanapalle").`,
  ...
  const [speechLang, setSpeechLang] = useState('en-IN'); // 'en-IN' or 'te-IN'
  ...
  const handleSend = async (overrideText) => {
    ...
    try {
      // Check if user clicked a saved address shortcut
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg h-[92vh] max-h-[720px] rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-slate-200/80 font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-extrabold text-base tracking-tight leading-tight">Fixvo AI Assistant</h3>
              ...
              <p className="text-[11px] text-blue-100 font-medium">Smart Voice & Service Booking</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/60">
          {messages.map((msg) => (
            ...
            >
              ...
              {/* Status query display card if any */}
              {msg.bookingSummary && (
                <div className="mt-3 w-full max-w-sm bg-white p-4 rounded-2xl border border-indigo-100 shadow-sm ml-9 space-y-2">
                  <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                    <span className="text-[11px] font-extrabold text-blue-600 uppercase tracking-wider">Active Booking</span>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 capitalize border border-blue-200">
                      {msg.bookingSummary.status}
                    </span>
                  </div>
                  <div className="text-xs text-slate-700 space-y-1">
                    <p><span className="font-bold text-slate-900">Service:</span> {msg.bookingSummary.service}</p>
                    <p><span className="font-bold text-slate-900">Technician:</span> {msg.bookingSummary.technician}</p>
                    <p><span className="font-bold text-slate-900">Location:</span> {msg.bookingSummary.address}</p>
                  </div>
                </div>
              )}

              {/* FIXVO AI BOOKING SUMMARY CARD */}
                ...
                <div className="mt-3 w-full max-w-sm ml-9 bg-white rounded-2xl border border-blue-200 shadow-md p-4 space-y-3 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className="pt-2 border-t border-slate-100 grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 py-2 px-3 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-bold rounded-xl transition-all cursor-pointer"
                    >
                      <Edit3 size={13} />
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-extrabold rounded-xl shadow-md shadow-blue-600/20 transition-all cursor-pointer"
                    >
                      <CheckCircle size={13} />
        ...
        <div className="p-3.5 bg-white border-t border-slate-200/80">
          ...
          >
            ...
            <input
              ...
              placeholder={isListening ? "Listening to your voice..." : (speechLang === 'te-IN' ? "తెలుగులో చెప్పండి లేదా టైప్ చేయండి..." : "Type or speak your service request...")}
            ...
            <button
              ...
              className="p-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:hover:bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-md shadow-blue-600/20 transition-all cursor-pointer"
            >
              <Send size={16} />
            </button>
          </form>
          ...
          {isListening && (
            <p className="text-[10px] text-rose-600 font-bold text-center mt-1.5 animate-pulse">
              🎙️ Listening ({speechLang === 'te-IN' ? 'Telugu' : 'English'})... Speak your issue and area now.
            </p>
          )}
        </div>
